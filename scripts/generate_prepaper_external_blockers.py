...
BLOCKERS = {
    ...
    # --- Local new experiments requiring frozen protocols (not yet run) ---
    "5.28": ("local agent (new frozen experiment)", "align with MQT Bench cross-abstraction data", "frozen MQT Bench version, license, input hashes, abstraction mapping", "ITT results + semantic verification on the aligned panel"),
    "5.29": ("local agent (new frozen experiment)", "run a Benchpress-style large software stress test", "pre-frozen scale tiers, timeout/memory caps, failure taxonomy", "time-to-solution + failure classification, not a smoke test"),
    "9.52": ("local agent (new telemetry experiment)", "measure time-to-first-valid with event-level traces", "frozen event-level trace schema + sampling clock", "per-run time-to-first-valid from event traces"),
    "9.53": ("local agent (new telemetry experiment)", "measure time-to-best with event-level traces", "frozen event-level trace schema + sampling clock", "per-run time-to-best from event traces"),
    "10.07": ("local agent (new telemetry experiment)", "measure energy consumption", "frozen energy metering methodology", "per-run energy measurements"),
    "10.08": ("local agent (new telemetry experiment)", "measure monetary cost", "frozen cost accounting methodology", "per-run cost records"),
    "13.12": ("local agent (new frozen experiment)", "test cross-qubit-range extrapolation beyond training 4-10", "frozen >10-qubit panel + fail-closed verification path", "out-of-range panel results with declared fail-closed path"),
    "13.17": ("local agent (new frozen experiment)", "add traceable real-world circuits", "real-application sources, license, provenance", "real-world circuit panel layered with custom generators"),
    "17.29": ("local agent (new frozen experiment)", "rerun core comparison without the custom benchmark", "external/real panel, same estimand", "custom-benchmark-free comparison results"),
    "17.30": ("local agent (new frozen experiment)", "estimate results on real circuits only", "real-circuit subset", "independent estimate + interval + worst family + ITT"),
    "18.04": ("local agent (new frozen experiment)", "build a third executable independent strong artifact", "a distinct executable external method, fixed version/config", "third artifact + semantic audit"),
    "18.06": ("local agent (new frozen experiment)", "add a large-circuit tier", "pre-registered scale, resource budget, verification tier, failure accounting", "large-tier results with failure accounting"),
