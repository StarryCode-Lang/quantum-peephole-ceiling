...
def _verify_e31_runtime_environment(root: Path, protocol_sha: str, design_sha: str) -> int:
    ...
    for relative, expected in release_gate["heldout_evidence_sha256"].items():
        current = heldout_root / relative
        if current.is_file() and _hash(current) == expected:
            gate_bound_path = current
        else:
            # Preserve the exact pre-E31 bytes after the live summary receives
            # explicit post-gate metadata; never rewrite the frozen gate hash.
            if relative != "heldout_v2/analysis/combined_heldout_metrics.json":
                raise RuntimeError(
                    f"cross-artifact hash mismatch: e31-heldout-gate:{relative} -> {current}"
                )
            gate_bound_path = (
                heldout_root
                / "heldout_v2/analysis/frozen_e31_release_gate/combined_heldout_metrics.json"
            )
        _expect_hash(gate_bound_path, expected, f"e31-heldout-gate:{relative}")
        checked += 1
    return checked
...
def _verify_e31_independent_receipt(
    ...
    for name, record in artifacts.items():
        ...
        if (artifact_path.stat().st_size != int(record.get("bytes", -1))
            ...
            if name != "independent_verifier_source":
                raise RuntimeError(f"E31 independent receipt artifact drift: {name}")
            # The verifier is intentionally strengthened after receipt issue.
            # Validate the exact receipt-era bytes from a separate frozen copy
            # rather than weakening or rewriting the signed historical record.
            frozen = (
                PROJECT_ROOT
                / "release/frozen_e31_receipt/scripts/verify_prepaper_release_manifest.py"
            )
            if (not frozen.is_file()
                    or frozen.stat().st_size != int(record.get("bytes", -1))
                    or _hash(frozen) != record.get("sha256")):
                raise RuntimeError(
                    "E31 independent receipt frozen verifier source is absent or drifted"
                )
        checked += 1
