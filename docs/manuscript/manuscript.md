...
> **Current status (2026-08-01):** This draft is being resynchronized to the
> 10.0.0-wave6 release. The historical v8 status note below is superseded for
> row counts and Phase-2b/listing-sensitivity status by the active evidence map
> and `docs/review/publication_readiness_2026-08.md`.
...
The experimental program comprises the registered experiments E1–E25 (36 canonical datasets, 82,789 result rows) plus five SOTA comparison suites (S1–S5) and a hardware-validation extension (EHW). Key experiments:
...
Data versioning: v1 (E1–E5, superseded), v2_fixed (E1–E5, canonical), v4 (E11–E13), v5 (E10, E14–E18), v6 (E19–E21, SOTA suites S1–S5), v7 (E22–E24, E29), v8 (E25 Phase-2b v2 — closed to the full-factorial depth grid at 2,427 rows in wave 6 — EHW noise-model validation, and the wave-5/6 listing-sensitivity dataset at 6,720 rows). Each artifact is stored with SHA-256 content hash, optimizer configuration JSON, circuit fingerprint, and random seed. v9 contains non-canonical current-code rerun and reconciliation evidence. See `docs/reproducibility.md` for the supported commands; no `run_experiments.py` entrypoint is claimed.
...
| Lemma 3 | Hybrid pipeline preserves unitary equivalence | All fidelity-verified rows | $F_{\text{avg}} = 1.0$ across the fidelity-verified corpus where exact/scalable checks are available (82,789 canonical rows, with documented exceptions) | MATCH |
...
All experimental data — raw CSV outputs, experiment metadata, and release manifests — are available in the project repository under `data/`, organized into canonical versioned subdirectories (v2_fixed–v8); v9 contains non-canonical rerun evidence. SHA-256 checksums for every dataset are recorded in `release/release_manifest.json`, which enumerates the 36 canonical datasets (82,789 result rows) analyzed here together with per-dataset row counts and schema versions. All datasets can be regenerated or verified with the commands in `docs/reproducibility.md`. No archival DOI is claimed in this working draft; a DOI will be added only after an actual archival deposit.

### Code Availability

The complete source code is publicly available at `https://github.com/StarryCode-Lang/quantum-peephole-ceiling`. The codebase includes optimizer implementations (`src/`), experiment drivers (`experiments/`), analysis tools (`analysis/`), and reproducibility scripts (`scripts/`). Python 3.12 dependencies are pinned in `requirements.txt` and `environment.yml`; a Docker container is provided via `Dockerfile`. Release version: v10.0.0-wave6.
