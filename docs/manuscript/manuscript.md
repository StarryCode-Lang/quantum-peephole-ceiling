...
Production compilers (Qiskit, Cirq, t|ket>) routinely achieve 10–40% gate-count reduction through sequences of local rewrite and synthesis passes, yet no prior study has, to our knowledge, systematically characterized *when* these passes succeed, *why* they sometimes fail entirely, or what representation-conditional limits constrain achievable reductions. This opacity encourages broad pass application, wasting computational resources on circuits that resist the tested local rewriting model while potentially missing opportunities for deeper optimization on amenable structures.
...
### 1.2 The Discovery: Listing-Model Sensitivity

A critical methodological observation emerged during the investigation: the circuit listing model — the data-structure ordering of gates — affects the action space visible to a flat-list peephole optimizer. We call this phenomenon **listing-model sensitivity**: the same quantum circuit, represented with different valid gate orderings, can produce different optimization outcomes under a restricted peephole model.
...
This seemingly innocuous data-structure choice has practical implications. Under the conflict-free LBL generator assumptions used in the formal statement, Theorem 1(b) gives an empty Phase-1 action space for $n \geq 2$: $\mathcal{S}_1(C) = \emptyset$ for every generated circuit $C$. The proof uses the layer-major index gap and the same-qubit gate predicate. The result is conditional on the exact generator/listing contract; it is not a universal statement about arbitrary layered circuit encodings or production compiler IRs. Consequently, $R_1(C) = 0$ for every Phase-1 optimizer applied to circuits satisfying those LBL assumptions.
...
**Contribution 1 (core): Listing-model sensitivity — formalization and empirical validation.**

- **What**: A formal characterization of how circuit listing order governs optimizer behavior. Theorem 1(b) proves that LBL makes the Phase-1 action space structurally empty for $n \geq 2$, explaining previously puzzling zero-variance experimental results. Theorem 1(a) gives the WCL adjacent-inverse-pair density $\mathbb{E}[|\mathcal{A}_{\text{adj}}(C)|] = n(d-1)\,p_{\text{cancel}}$ with $p_{\text{cancel}} = (1-\rho)^2 k_1/g_1^2 + \rho^2/(2 g_2 (n-1))$ (even $n$; odd-$n$ corrections stated in §3.2), directly validated by E30 (13,500 trials, 27 cells, median relative error 1.4%).
...
- **Why**: Provides, to our knowledge, the first systematic characterization of when peephole optimization succeeds and fails across diverse circuit families, with circuit-family optimization prescriptions for compiler design. The stratification is not a gradual spectrum but a categorical division determined by algebraic structure — and the Phase-2b v2 experiment (E26) shows that the boundary of the "incompressible" class is itself move-set-dependent: IQP moves from 0.5% (Phase-2a) to 92.0% (Phase-2b v2), Structured from 0.0% to 40.2%, and the BV oracle family to its provable $k+2$ optimum (69.2% mean over secrets), the single most citable result of this study.
- **Evidence**: 96,289 rows across 37 canonical datasets spanning 15 circuit families and 6 optimizer types (E1–E30), supplemented by E23/E24 theory-validation experiments (235 trials), E20/SOTA multi-compiler comparison (Qiskit/Cirq/t|ket>, 10 trials per cell, fair no-coupling-map mode), and E26 Phase-2b full-scale validation (2,427 rows, fidelity $\geq 1 - 2\times10^{-13}$ on every row). The scale exceeds prior efforts — Quartz (~100 benchmarks), Quarl (~50 benchmarks), Micro-Benchmark Suite (6 circuits) — by two orders of magnitude; the comparative literature claim still requires external verification.
...
Section 2 provides background on the quantum circuit model, listing models, peephole optimization, and quantum compilers. Section 3 presents the core contribution: listing-model sensitivity, including formal definitions (Theorem 1), the E19 flagship experiment, and practical implications. Section 4 develops the theoretical framework: Phase-1/2 taxonomy, reduction ceiling results, and context-dependent advantage constructions. Section 5 describes experimental methods. Section 6 presents empirical results organized around the three research questions. Section 7 discusses implications, limitations, and future work. Section 8 concludes.
...
We study 15 primary circuit families spanning random, algorithmic, variational, and error-correcting circuits. The roster below is the benchmark suite used uniformly by the 15-family experiments (E12–E22, E26) and is identical to the roster classified in §6.3:
...
Two naming notes keep the roster consistent across experiments. (i) The synthetic *Universal random* ensemble of the early listing experiments (E1–E5, E10, E19) is the same generator family reported as **HaarRandom** in the 15-family benchmark suite; the Phase-2b v2 experiment (E26) reports it under the name *Universal*. (ii) The *Structured* brickwork family appears in the random-ensemble experiments (E10) and in E26 (40.2% under Phase-2b v2), but is not part of the 15-family benchmark suite, which instead covers the variational UCCSD family; E26 therefore covers 16 families (the 15 of Table 2 with Universal and Structured explicit, minus HaarRandom).
...
The Micro-Benchmark Suite [Merilehto 2025] evaluates six self-generated circuits (3–8 qubits) across Qiskit, t|ket>, Cirq, and Amazon Braket, measuring post-routing depth and gate counts. However, it is a measurement tool, not a theoretical framework: it reports empirical metrics without explaining *why* different compilers produce different results or characterizing structural properties that determine optimizability. Its corpus (6 circuits) is orders of magnitude smaller than our study (96,289 rows across 37 canonical datasets, 15 families). Other benchmarking efforts — MQT Bench [Quetschlich et al. 2023] and QASMBench [Li et al. 2022] — focus on circuit diversity rather than optimization-theoretic analysis. On the optimizer side, VOQC [Hietala et al. 2021] proves peephole rewrites correct in a proof assistant, Quartz/Quanto automate identity discovery, and equality saturation [Yang et al. 2026] and SSR [Huang et al. 2025] are the closest recent peephole-class systems; Table 12 (§6.5) places our prototype against production compilers on identical circuits. Despite these advances, none of the existing optimization frameworks — Quartz, Quanto, Quarl, AlphaTensor-Quantum, Relaxed Peephole Optimization, VOQC, equality saturation, SSR, or ZX+RL — provides a prototype action-space ceiling characterization, quantifies context-dependent Phase-2 advantage, or offers a systematic multi-compiler benchmark with formal theory. The claim of filling this gap requires a fresh, independently verified literature search before submission.

**Classical compiler connection.** The comparison is about where a rewrite pass
gets its context, not about a one-to-one equivalence between classical and
quantum intermediate representations. McKeeman's peephole pass and Massalin's
superoptimizer operate on bounded instruction windows [McKeeman 1965; Massalin
1987], while intermediate-code systems such as Tanenbaum et al. [1982] make the
representation part of the optimization interface. Our flat-list prototype is
the quantum analogue of a window whose visibility depends on instruction order;
Qiskit's DAG, pytket's DAG, and Cirq's moments make wire or component structure
explicit before local rewriting. This distinction explains why production
compilers can be invariant to flat-list permutations without requiring an
explicit WCL pass, and it limits our listing-sensitivity claim to optimizers
whose windows are defined on a flat listing.

---

## 3. Listing-Model Sensitivity

This section presents the core contribution of this work: the formalization and empirical validation of listing-model sensitivity — the finding that circuit instruction ordering changes the action space visible to a restricted Phase-1 peephole optimizer.
...
**(a) Wire-consecutive listing.** Let $C(n, d, \rho)$ be a random circuit on $n$ qubits of depth $d$ with two-qubit gate density $\rho$, represented in WCL, generated by the layer model: in each layer qubits are randomly paired, each pair receives a two-qubit gate with probability $\rho$ (uniformly from $\mathcal{G}_2$, $|\mathcal{G}_2| = g_2$), and every uncovered qubit receives a one-qubit gate drawn uniformly from $\mathcal{G}_1$ ($|\mathcal{G}_1| = g_1$), of which $k_1$ are discrete gates with exact inverses in the set (continuous rotation families contribute measure-zero inverse matches). For even $n$, the expected number of listing-adjacent inverse pairs is:

$$\mathbb{E}[|\mathcal{A}_{\text{adj}}(C)|] = n(d-1) \cdot p_{\text{cancel}}(n, \rho), \qquad p_{\text{cancel}}(n, \rho) = (1-\rho)^2 \cdot \frac{k_1}{g_1^2} + \frac{\rho^2}{2\,g_2\,(n-1)}.$$

For odd $n$ the random-permutation matching leaves one qubit unpaired per layer, replacing $\rho$ in the one-qubit term by $\rho(n-1)/n$ and the pair-coverage probability $1/(n-1)$ by $1/n$; E30 validates both corrections.

*Proof (corrected 2026-08-06).* On a fixed qubit wire, two consecutive layers both place single-qubit gates with probability $(1-\rho)^2$ (even $n$); conditional on placement, the second gate is the exact inverse of the first with probability $\sum_{g \text{ discrete}} (1/g_1)(1/g_1) = k_1/g_1^2$, since each discrete gate has exactly one inverse in the set and continuous rotations match with probability zero. For two-qubit gates, a given qubit pair is covered in both layers with probability $(\rho/(n-1))^2$ and the gates coincide (hence cancel, both $g_2$ gates being self-inverse) with probability $1/g_2$; summing over the $\binom{n}{2}$ pairs and $d-1$ layer pairs gives $(d-1)\binom{n}{2}\rho^2/((n-1)^2 g_2) = n(d-1)\rho^2/(2g_2(n-1))$. Adding the one-qubit contribution $n(d-1)(1-\rho)^2 k_1/g_1^2$ by linearity of expectation yields the result. Each cancellation removes 2 gates from $|C| \approx nd(1-\rho/2)$, so $\mathbb{E}[R_{\text{adj}}] \leq 2\,\mathbb{E}[|\mathcal{A}_{\text{adj}}|]/|C|$. *Correction note:* the original statement omitted the factor $k_1$ (writing $(1-\rho)^2/g_1^2$, with a claimed bound $p_{\text{inv}}^{(1q)} \leq 2/g_1^2$ that fails for $g_1 \geq 3$) and double-counted two-qubit pairs once per endpoint wire; for the standard gate set $k_1 = 8$, $g_1 = 11$, $g_2 = 2$. The corrected formula is directly validated by E30 (13,500 trials, 27 cells, max $|z| = 2.86$, median relative error 1.4%). $\blacksquare$
...
Experiment E19 is the cleanest demonstration of listing-model sensitivity. It applies a wire-traversal preprocessing step to the same random universal circuits used in E1, converting from LBL to WCL while preserving unitary equivalence.
...
**Phase2bTemplateMatcher (v2.0.0, Phase-2b).** Template-assisted optimizer combining inverse-cancellation closure over self-inverse, named-inverse, and parametric gate pairs; phase-polynomial merging; and the standard $\leq$3-qubit Clifford conjugation template set. The template library is the inverse-closure-plus-conjugation set, *not* an exhaustive enumeration of the 11,520-element two-qubit Clifford group. Used in E26 (§6.4).
...
Two further protocol choices follow `docs/results/experimental_design.md` §8. Confidence intervals use the *percentile* bootstrap rather than BCa: the BCa jackknife acceleration estimate is unstable for the degenerate, near-constant reduction distributions typical under LBL (a point mass at 0%), and at $N \geq 100$ per condition the percentile/BCa difference is negligible. For zero-inflated outcomes, we report zero mass and conditional nonzero summaries using `analysis/phase1_statistics/zero_inflated.py`; Pearson correlations and variance-based effect sizes remain undefined when their inputs are degenerate. Effect sizes are interpreted against pre-registered thresholds — Cohen's $d$: $<0.2$ negligible, $0.2$–$0.5$ small, $0.5$–$0.8$ medium, $\geq 0.8$ large; Cliff's $\delta$: $<0.147$ negligible, $0.147$–$0.33$ small, $0.33$–$0.474$ medium, $\geq 0.474$ large — with Hedges' $g$ reported for small per-cell samples.

Fidelity verification: exact average gate fidelity for $n \leq 12$ (full $2^n \times 2^n$ unitary matrices, $O(4^n)$); a global-Haar Monte Carlo estimator using 1,000 random states for $n > 12$; and exact stabilizer tableau comparison for Clifford circuits ($O(n^2)$). The sampled estimate is approximate and its uncertainty is outcome-dependent; 1,000 samples is not a universal 3% guarantee. Across experiments where fidelity was successfully computed, optimizer outputs preserve unitary equivalence. Documented failure rows (especially E18 decomposition/fidelity failures) are tracked and filtered from analyses requiring valid fidelity; E18 additionally reports an intention-to-treat analysis over all attempted circuits (§6.8). A known labeling issue in E21 — rows computed with the fallback estimator (`target = None`) are tagged `exact_average_gate_fidelity` — is disclosed in §7.5.
...
The experimental program comprises the registered experiments E1–E30 (37 listed canonical datasets, 96,289 result rows; 35 active — 96,205 rows — excluding 2 superseded provenance entries) plus five SOTA comparison suites (S1–S5) and a noise-model validation extension (EHW). E31 is a non-canonical smoke pilot recorded separately under `data/v11/e31_listing_phase2b/`; it is not part of the release registry. Key experiments:
...
| E11 | Real-Circuit Benchmark | 519 | 15 | P1+2 |
| E12 | Compiler Baseline (Qiskit) | 568 | 15 | P1+2 |
| E13 | Structural Ceiling Proxy | 56 | 7 | P1+2 |
| E14 | Extended Benchmark | 2,130 | 15 | P1+2 |
| E15 | Multi-Compiler (Qiskit only; Cirq/t\|ket> in E20) | 994 | 15 | P1+2 |
...
| E25 | Industry Benchmark Proxies (Qiskit) | 66 | industry proxies | P1+2 |
| E26 | Phase-2b Full-Scale Validation | 2,427 | 16 | P2b |
| E29 | Multi-Seed Optimizer Stability | 800 | 1 (Universal) | P1 |
| E30 | Theorem 1(a) WCL Density Validation | 13,500 | 1 (layer model) | P1 |
| EHW | Noise-Model Validation (fake backends) | 288 | 3 circuits | P1+2 |

Data versioning: v1 (E1–E5, superseded), v2_fixed (E1–E5, canonical), v4 (E11–E13), v5 (E10, E14–E18), v6 (E19–E21, E25, SOTA suites S1–S5), v7 (E22–E24, E29), v8 (E26 Phase-2b v2 — closed to the full-factorial depth grid at 2,427 rows in wave 6 — E27 new families, EHW noise-model validation, and the wave-5/6 listing-sensitivity dataset at 6,720 rows), v10 (E30 Theorem 1(a) validation, canonical; E23 fixed-generator verification rerun, non-canonical), and v11 (non-canonical E20 corrected rerun and E31 pilot). Each artifact is stored with SHA-256 content hash, optimizer configuration JSON, circuit fingerprint, and random seed. v9 contains non-canonical current-code rerun and reconciliation evidence (consolidated disposition: `docs/review/wave6/reconciliation_disposition.md`). See `docs/reproducibility.md` for the supported commands; no `run_experiments.py` entrypoint is claimed.
...
### 6.2 Listing-Model Sensitivity: WCL vs LBL (E19, RQ1)
...
**Table 11. BV instances versus the Theorem 9 bound (E26, 80 instances, $n = 3$–$10$, 10 secrets per size).**
...
**Table 11b. BV reduction by size versus the Theorem 9 bound (E26; 10 secrets per size).**
...
Table 19 maps every formal claim of Sections 3–4 to its experimental test (full per-row notes in `docs/manuscript/claim_evidence_table.csv`; the historical narrative is in `docs/results/analysis_summary.md`).

**Table 19. Cross-validation of formal claims against experiments.**

| ID | Prediction (abbreviated) | Experiment(s) | Observed | Status |
|:---|:---|:---|:---|:---|
| Thm 1a | WCL: $\mathbb{E}|A_{\text{adj}}| = n(d-1)\,p_{\text{cancel}}$ | E30 (13,500 trials); E1–E5 | E30: 27 cells, max $\|z\| = 2.86$, median rel. err. 1.4% (corrected constants); aggregate count distribution has 0/27 BH-rejected Poisson GOF cells; wire-level independence is not identifiable from stored aggregates; E1–E5 LBL zeros consistent with Thm 1b | MATCH (formula corrected and directly validated 2026-08-06; distribution diagnostic in `docs/analysis/e30_distribution_p10.md`) |
...
| Lemma 3 | Hybrid pipeline preserves unitary equivalence | All fidelity-verified rows | $F_{\text{avg}} = 1.0$ across the fidelity-verified corpus where exact/scalable checks are available (96,289 canonical rows, with documented exceptions) | MATCH |
...
| Thm 9 | BV: $R_{1+2b} \geq n/(4.5n+4)$ | E26 (80 instances) | Exact $k{+}2$ optimum; 3.1–4.2× bound | MATCH (exceeds bound) |
| C1 | Phase-1 ceiling is structural | E1–E5, E10, E14, E23 | 0% across all empty-$\mathcal{S}_1$ families | MATCH |
| C2 | Phase-2 context-dependent $\Omega(1)$ | E10, E14, E16, E24, E26 | Oracle/Clifford/IQP/BV ≫ 0; QFT/GHZ = 0 | MATCH |

**Scorecard:** 14 MATCH (incl. 2 trivial-bound matches), 3 PARTIAL (Prop 1, Thm 8a, Thm 8b), 0 MISMATCH, 0 UNTESTED. The former Thm 1a mismatch had two components: a model-assignment issue (the formula assumes WCL while E1–E5 use the LBL generator, whose correct prediction is Thm 1b's exact zero) and two constant errors in the original statement (missing $k_1$ factor; double-counted two-qubit term). Both were resolved on 2026-08-06: the corrected formula is stated in §3.2 and directly validated by E30.

**The Thm 1a resolution in detail.** For E1 parameters ($n = 5$, $\rho = 0.3$), the *original* Thm 1a expectation $p_{\text{cancel}} = (1-\rho)^2/25 + \rho^2/4 = 0.0421$ (computed with that era's generator constants) predicts $\mathbb{E}|A_{\text{adj}}| \approx 0.84$ pairs at $d = 5$ and $\approx 10.3$ pairs at $d = 50$, where a Poisson approximation assigns probability $\sim$0% to observing zero — yet all 25,000 trials contain exactly zero adjacent inverse pairs. The resolution is structural: the LBL generator places gate $(q, L)$ at listing index $Ln + q$, so same-qubit gates are always separated by $n - 1$ intervening gates and listing-adjacency is impossible for $n \geq 2$ (Thm 1b). The mismatch is therefore the cleanest possible illustration of this paper's thesis: the same formula, the same circuits, and two listing models yield contradictory predictions, and only the listing-aware prediction (Thm 1b) matches measurement. The WCL side is now directly tested too: E30 generates circuits under the theorem's exact layer model, re-lists them in WCL, and confirms the corrected density prediction across 27 $(n, d, \rho)$ cells (max $|z| = 2.86$; median relative error 1.4%), while E19 provides the corresponding WCL reduction measurement on random circuits (7.83% > 0).

**Figure 18.** Direct validation of the corrected Theorem 1(a): empirical mean adjacent-inverse-pair counts versus the theoretical prediction for all 27 E30 cells (colors: $\rho$; markers: $n$). Points lie on the identity line within sampling noise. *(`analysis/figures/fig18_e30_thm1a_validation.pdf`, generated by `analysis/figure_e30.py` from `data/v10/e30/`)*
...
**Figure 19.** Connectivity versus ceiling behavior (E17): net reduction by topology for reducible and ceiling families. *(`analysis/figures/fig14_connectivity_ceiling.pdf`)*
...
2. **WCL validation scope.** E19 validates WCL on random Universal circuits, and E19-extended (960 rows, §6.2) provides a first cross-family check — 7 of 16 families gain from WCL (8 of 16 counting the saturated CNOT_chain family), the remaining nine show no benefit. E31 provides a non-canonical smoke probe of WCL/SHUFFLE crossed with Phase-2b, but full WCL shuffle controls and WCL $\times$ Phase-2b interactions across family, size, depth, and seed remain untested. If WCL changes the ceiling on additional structured families under those controls, the LBL-based classification may be partially an artifact.
...
9. **Shuffle-control scope.** E22's random-shuffle control (answering an earlier limitation) covers 16 families for Phase-1/Phase-2; it shows shuffling *increases* Phase-1 reduction, so the original ceiling claims are conservative. E31 adds one random topological listing per smoke circuit and crosses it with Phase-2b, but higher-replication shuffle controls and the full WCL/Phase-2b factorial remain untested.
...
20. **Rerun reconciliation (wave 6).** All eleven experiments carrying 'source modified since run' warnings were rerun under the current codebase and reconciled row-by-row against canonical (`docs/review/wave6/rerun_batch1.md`–`rerun_batch4.md`; consolidated disposition in `docs/review/wave6/reconciliation_disposition.md`): E12 (560/560 overlap-identical), E13 (overlap 56/56 identical), E14 (2,072/2,130 rows identical; 38 IQP commutation/hybrid rows higher under current code; 20 canonical fidelity = 0.0 rows on unchanged circuits are artifacts that current code correctly reports as 1.0), E15 (978/986 identical; 8 IQP rows attributable), E16 (680/696 overlap-identical; 16 IQP rows attributable), E17 (input-identical rows reproduced exactly, 54/54; wide input drift from generator/topology changes), E18 (rerun diverged via generator drift: BasisTranslator now decomposes formerly-erroring families; 172/192 shared reductions agree; canonical retained), E19 (10,000/10,000 value-identical; the WCL 7.83% vs LBL 0% headline reproduces exactly), E20 (1,070/1,070 bit-for-bit identical), and E21 (1,140/1,140 input-identical; 40 IQP rows attributable; the "time saved without reduction loss" conclusion unchanged). Every observed difference is fully attributable to post-canonical source changes — predominantly the strengthened commutation predicate (review FATAL-1 repair direction) and improved BasisTranslator coverage, i.e. optimizer-capability enhancements rather than data errors — so canonical data are retained for all eleven experiments by adoption decision, with E14/E15/E17/E21 annotated. **Unified IQP sensitivity statement:** IQP-family commutation/hybrid reduction values quoted from canonical E14/E15/E16/E21 are systematically conservative under the current codebase.
...
5. **Production compiler representation study (audit completed; relisting coverage completed).** The audit (`docs/analysis/compiler_listing_audit.md`) establishes that Qiskit, Cirq, and t|ket> are flat-listing invariant because their DAG/moment windows are wire-relative — WCL-equivalent by representation, not by relisting. The controlled relisting experiment now covers all 15 families (6,720 rows, 168 family–$n$–tool combinations; wave 6): production compilers (Qiskit L3, pytket FPO no-swap, Cirq) are sensitive in 0/126 combinations, while the flat-list prototype is sensitive in 15/42 combinations across 6 families (UCCSD_inspired added in wave 6). Honest gap: qwalk_8 is complete at 20/20 variants, but all 80 nine-qubit rows skip exact Operator construction under the documented budget; relisting is unitary-preserving by construction and qwalk_3/qwalk_5 pass exact checks. Same-wire commuting-swap relistings remain unenumerated. Remaining work: audit whether beyond-peephole passes (resynthesis, phase polynomials) interact with listing choice.
...
This paper introduces **listing-model sensitivity** — the finding that circuit instruction ordering changes the action space visible to a flat-list peephole optimizer. Under layer-by-layer listing (LBL), the Phase-1 action space is provably empty for $n \geq 2$ under the stated generator assumptions (Theorem 1(b)), yielding exactly 0.0000% reduction across 25,000 random circuits; under wire-consecutive listing (WCL), the same circuits exhibit 7.83% mean reduction (E19, 10,000 rows); and a random listing shuffle causally *increases* Phase-1 reduction (E22). Representation choice, more than algorithm choice, determines what this restricted peephole model can expose. The claim is scoped to listing-based peephole designs: a 15-family relisting audit (6,720 rows, 0/126 sensitive production-compiler combinations) shows that Qiskit, Cirq, and t|ket> escape listing dependence *representationally* — their DAG/moment windows are wire-relative, equivalent to compiling against the wire-consecutive listing — so for production compilers the actionable content of this work is the class taxonomy and escalation map, not relisting (`docs/analysis/compiler_listing_audit.md`).

Across 37 canonical datasets (96,289 rows, 15 circuit families, 6 optimizer types, 3 production compilers), we identify a categorical stratification of circuit families: trivially compressible (CNOT, 100%), commutation-enabled (Oracle/Clifford/Grover), genuinely structural ceilings (QFT, GHZ, SurfaceCode — unanimous across all tested compilers), prototype-limited ceilings (VQE, HardwareEfficient, IQP, UCCSD, QAOA — 6.3–64.4% recoverable by production peephole passes), and practical ceilings (Adder, QuantumWalk — every tested tool fails). The Phase-2b template matcher breaks several prototype ceilings at full scale (E26), reaching the **exact $k+2$ gate optimum on all 80 Bernstein–Vazirani instances** — exceeding the rigorous Theorem 9 bound by 3.1–4.2× — and 92.0% on IQP, while leaving the genuine and practical ceilings untouched. Theory and experiment agree across 17 cross-validation rows (14 match, 3 partial, 0 mismatch, 0 untested). A ceiling-aware pipeline skips futile phases with bitwise-identical output (up to 227.8× per-family speedup; 34.97× aggregate under prototype timing, 2.20× under symmetric timing), while its repaired hybrid predictive model remains exploratory because of post-hoc selection and held-out mechanism failures (§6.6).
...
All experimental data — raw CSV outputs, experiment metadata, and release manifests — are available in the project repository (`github.com/StarryCode-Lang/quantum-peephole-ceiling`) under `data/`, organized into canonical versioned subdirectories (v2_fixed–v8, v10); v9 contains non-canonical rerun evidence. SHA-256 checksums for every dataset are recorded in `release/release_manifest.json`, which enumerates the 37 canonical datasets (96,289 result rows; 35 active excluding 2 superseded provenance entries) analyzed here together with per-dataset row counts and schema versions. All datasets can be regenerated or verified with the commands in `docs/reproducibility.md`. No archival DOI is claimed in this working draft; a DOI will be added only after an actual archival deposit.

### Code Availability

The complete source code is publicly available at `https://github.com/StarryCode-Lang/quantum-peephole-ceiling`. The codebase includes optimizer implementations (`src/`), experiment drivers (`experiments/`), analysis tools (`analysis/`), and reproducibility scripts (`scripts/`). Python 3.12 dependencies are pinned in `requirements.txt` and `environment.yml`; a Docker container is provided via `Dockerfile`. Release version: v10.1.0-audit-fixes.
...
F. S. conceived the study, designed and executed all experiments, implemented the optimizers and analysis pipeline, performed the theoretical derivations, and wrote the manuscript.

### Acknowledgments

Experiments were executed on a single workstation (16 GB RAM, no GPU); the exact-fidelity verification envelope (n ≤ 12) and the noise-model-only hardware study (EHW) reflect that compute budget. The author used AI-assisted programming tools for code development, documentation sweeps, and reference verification; all scientific claims, experimental designs, and results were produced and verified by the author, and all AI-assisted outputs were reviewed before integration.
...
[7] M. Amy, D. Maslov, M. Mosca, and M. Roetteler, "A meet-in-the-middle algorithm for fast synthesis of depth-optimal quantum circuits," IEEE Transactions on Computer-Aided Design of Integrated Circuits and Systems, vol. 32, no. 6, pp. 818–830, 2013. arXiv:1206.0758
...
[9] M. Amy, D. Maslov, M. Mosca, and M. Roetteler, "Polynomial-time T-depth optimization of Clifford+T circuits via matroid partitioning," IEEE Transactions on Computer-Aided Design of Integrated Circuits and Systems, 2014. arXiv:1303.2042
...
[12] D. Janzing, P. Wocjan, and T. Beth, "Non-identity-check is QMA-complete," International Journal of Quantum Information, vol. 1, no. 4, pp. 507–518, 2003. arXiv:quant-ph/0305050
...
[26] D. Shepherd and M. J. Bremner, "Temporally unstructured quantum computation," Proceedings of the Royal Society A, vol. 465, pp. 1413–1439, 2009. DOI: 10.1098/rspa.2008.0443; arXiv:0809.0847
...
[32] S. Yamashita and I. L. Markov, "Fast equivalence-checking for quantum circuits," Quantum Information and Computation, vol. 10, no. 9–10, pp. 721–734, 2010. arXiv:0909.4119

[33] R. Iten et al., "Exact and practical pattern matching for quantum circuit optimization," ACM Transactions on Quantum Computing, vol. 3, no. 1, pp. 1–44, 2022. arXiv:1909.05270
...
[41] N. Quetschlich, L. Burgholzer, and R. Wille, "MQT Bench: Benchmarking software and design automation tools for quantum computing," Quantum, vol. 7, 1062, 2023. arXiv:2204.13719
...
[47] Y. Huang et al., "SSR: A swapping-sweeping-and-rewriting optimizer for quantum circuit transformation," ACM Transactions on Design Automation of Electronic Systems, 2026. DOI: 10.1145/3828549. arXiv:2503.03227 (preprint 2025).
