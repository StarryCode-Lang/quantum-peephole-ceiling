...
|----------|----------------------|------------------------|-------------|-------------|
| Phase-1 ceiling (random circuits, LBL) | R1 = 0 for n >= 2 (Thm 1b) | 0.0000% mean, std = 0.0000, max = 0.0000 (E1, 25,000 trials) | Consistent | Exact match. LBL listing structurally empties S1(C); zero variance confirms structural (not statistical) ceiling |
...
| BV oracle Phase-2b lower bound | R_{1+2b} >= n/(4.5n+4) >= 2/13 = 15.4% (Thm 9) | 13.98% Phase-2a on Oracle (E14); structural ceiling 20.54% (E13, BV) | Consistent | Phase-2a (commutation only) is a strict subset of Phase-2b (+ templates); 13.98% Phase-2a is near the 15.4% Phase-2b bound, with the residual gap attributable to unimplemented H-CNOT-H template matching |
| Structural ceiling tightness | Ceiling is listing/window/model-conditional (Conj. C1) | Mean ceiling gap = ~0.00% across 56 circuits / 7 families (E13); BV: ceiling 20.54% = observed 20.54% | Consistent | Observed reduction matches structural upper-bound estimate; zero gap confirms ceiling is structural, not algorithmic |
