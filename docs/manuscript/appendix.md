...
> **Active-status note (2026-08-06):** This appendix contains wave-history
> tables (V6-era claim map below, kept for provenance). Current evidence is
> governed by `docs/manuscript/claim_evidence_table.csv`,
> `data/DATA_CANONICAL.md` (v2.4.0), and `release/release_manifest.json`. The
> active release has 37 canonical datasets and 96,289 rows (E1–E30); the
> Phase-2b full-scale experiment is numbered **E26** (formerly "E10p2b-v2" /
> "E26_phase2b_full_v8"; the former 4-row E26 BV pilot is "E26-pilot",
> superseded), and **E30** directly validates the corrected Theorem 1(a) WCL
> density formula (13,500 trials). Phase-2b v2 is full-scale evidence,
...
| C5 | Hardware topology constraints affect optimization opportunities differently. | E17 | Fig. 19 | Linear, grid, heavy-hex topologies on all 15 families | Simplified topology models | Real hardware has additional noise/error effects not modeled. |
...
> **Added**: 2026-07-21 (wave 2). **Data**: `data/v6/sota_benchmark/` (canonical raw runs in `raw/`, 520 rows per tool/config, 10 trials per (family, n) cell, seed 42 + 1000*trial; environment record `metadata/hardware_environment.json`: i7-11370H, qiskit 2.4.1, pytket 2.18.0, cirq 1.6.1). **Manuscript tables**: integrated into manuscript §6.5 (Tables 12-16); the former auto-generated fragment `docs/manuscript/sections/sota_table_fragment.md` was removed in the 2026-08-06 cleanup and can be regenerated with `experiments/generate_sota_tables.py` if needed. **Full results document**: `docs/results/sota_compiler_benchmark.md`. **Manuscript cross-reference**: §2.7 (related work), §6.4 (multi-compiler mechanism analysis).
