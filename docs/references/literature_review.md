...
> **This file contains the narrative literature review (43 core references). For the authoritative reference list used in the manuscript, see unified_references.md.**

> **Document Status**: Comprehensive literature review for publication-quality manuscript.  
> **Version**: 1.5 (audit correction)
> **Date**: 2026-08-09
> **Scope**: Systematic review of quantum circuit optimization, peephole optimization, complexity theory, compiler frameworks, and recent advances (2021–2026).
...
> **Innovation audit correction (2026-08-09):** Broad claims that search-space
> limitation, representation choice, or local-optimality guarantees are absent
> from prior work are not defensible. Quarl explicitly motivates a graph state
> representation and decomposed action space; Quasar uses complementary graph
> and sequence e-graphs and gives step-limited optimality; Arora et al., *Local
> Optimization of Quantum Circuits* (arXiv:2502.19526), prove a cut-and-meld
> local-optimality guarantee. The project's potentially distinctive claim is
> narrower: a systematic empirical decomposition of **flat-listing
> linearization sensitivity** and the measured ceiling of its specified local
> rule set. Record counts are not comparable to another paper's number of
> unique benchmark circuits, so all "largest" and "two orders of magnitude"
> priority claims in this review are withdrawn.

> **Search update (2026-08-10):** SciSpace surfaced Q-PreSyn
> (arXiv:2601.19738); Sider's direct arXiv search and the official arXiv record
> independently confirmed it. Q-PreSyn explicitly treats representation
> dependence of local synthesis and learns equivalence-preserving pre-synthesis
> edits. This removes representation dependence itself from the candidate
> novelty; only the project's narrower diagnostic and sealed transfer test
> remain candidates.
...
**Janzing, Wocjan & Beth (2003/2005)** proved QMA-completeness for the
standard promise problem **Non-Identity Check** (and a promise version of
non-equivalence checking). The norm, global-phase quotient, promise gap, and
YES/NO orientation are essential; a bare “are two circuits close?” schema is
not automatically the same problem.

**Kempe, Kitaev & Regev (2006)** established QMA-completeness of the
**$k$-Local Hamiltonian problem** for the stated locality regimes. This does
not by itself supply a circuit-size-gap reduction to the project's CODP.

**Implication boundary**: hardness of a semantic promise problem does not imply
hardness of a particular optimization problem merely because equivalence is
checked inside an optimizer. A valid CODP hardness result needs an explicit
promise-preserving reduction with a circuit-size gap; none is established here.
...
**Aaronson & Gottesman (2004)** support polynomial-time Clifford simulation and
tableau/equivalence operations:
- Clifford circuits can be represented as binary symplectic matrices
- constructive normal forms can be produced efficiently under stated models
- this does **not** imply globally minimum gate count or depth for every basis,
  connectivity, ancilla policy, and cost metric

**Our scoped observation**: the corrected restricted AG-stage generator has
zero Greedy reduction in v10 E23. This is a property of that generator/listing
and Greedy predicate, not a consequence of Gottesman--Knill and not a claim
that general Clifford circuits are already optimal.
...
**Open direction**: treewidth and non-Clifford count are plausible parameters,
but this project proves neither FPT membership nor W[1]-hardness. Efficient
Clifford equivalence likewise does not imply polynomial-time globally optimal
Clifford synthesis.
...
| **Our work** | Phase 1 + Phase 2 characterization | Configuration/family dependent | **Empirical boundary under specified listing/window/rules; scoped existence theorems** | CODP complexity open |

**Key insight**: Existing compilers and recent optimizers provide both practical reductions and several kinds of search/local-optimality guarantees. This project contributes a more specific diagnostic: a **listing/window/model-conditional empirical ceiling** for the tested local rule set. Whether that diagnostic is novel after Quasar and cut-and-meld local optimization requires a feature-by-feature comparison, not an absence claim.
...
|-----|--------------|----------------|
| **QMA-hardness of peephole optimization** | Related identity-check problems have QMA results | **Open here**; empirical records cannot establish computational hardness and the draft reduction is withdrawn |
| **Structural ceiling characterization** | Related action-space, representation, equality-saturation, and local-optimality analyses exist | Narrow flat-listing/rule-set definitions (D9–D10); empirical measurement across 6 experiments; general theorem not established |
...
| **Average-case complexity** | Related random/benchmark analyses exist | Empirical behavior for one generator and rule set; not an average-case complexity result |
...
| Gap | Current State | Our Contribution |
|-----|--------------|----------------|
| **Scale of random circuit trials** | Protocols and units vary across studies | **25,000 repeated E1 trials**; no priority claim without a reproducible systematic review |
...
| **Benchmark suite scale** | Benchmark definitions and repetition counts differ | **96,205 active-canonical rows in 35 datasets**; rows are repeated observations and are not unique-circuit counts |
...
Non-Identity Check (Janzing et al.)     →    CODP hardness remains open here
Clifford simulation (Gottesman)         →    Not a circuit-optimization theorem
...
RL-based optimization (Quarl, ZX+RL)    →    We measure failure of this local rule set only
...
The project's remaining candidate contribution is narrower than the recent
frameworks surveyed in Section 10.  It is a reproducible empirical diagnostic
of sensitivity to one specified flat listing, window policy, and local rewrite
class, together with a sealed out-of-family predictive test.  Novelty is not
established until that diagnostic is compared feature-by-feature with Quasar,
cut-and-meld local optimization, PCOAST, Quartz, and Quarl; no general formal
theory or superiority claim is made here.

1. **Representation-boundary diagnostic**: paired quantification of how a
   specified flat listing changes the visible local action space and Greedy
   reduction under a frozen rule set.
2. **Context-dependent Phase-2 estimates**: family-stratified effect estimates
   plus scoped constructive Phase-2a/2b existence results. No “first” or
   assumption-of-unconditional-benefit claim is supported by the comparator
   review.
3. **Sealed out-of-family prediction**: a frozen structural classifier tested
   on eight new generator families after prediction sealing; this is evidence
   for transfer of the diagnostic, not universal optimizer selection.
4. **Large-Scale Empirical Validation**: 96,205 active-canonical rows across 35 datasets (experiments E1–E30), spanning 15 circuit families and 6 optimizer types. This is substantial repeated-measures evidence, but it cannot be compared numerically with Quartz/Quarl unique benchmark counts and carries no "largest study" claim.
5. **Shared-contract compiler comparison**: 520 frozen inputs, exact equivalence
   validation, common-basis normalization, and ITT failures for fixed versions
   of custom/Qiskit/Cirq/t|ket>. It is a reproducible benchmark, not a first or
   universal ranking.
6. **Auditable data protocol**: versioned data, failure retention, source/input
   hashes, deviation logs, and sealed held-out predictions. No cross-study
...
1. **Classify a fully specified CODP promise problem**: define the channel
   metric, gate/cost/ancilla model, promise orientation and size gap, then seek
   a valid reduction or algorithm. This is unrelated to the refuted former C1
   stochastic-ceiling conjecture.
...
**Boundary relative to this project**: Quanto studies identity discovery and
application, while this project measures representation/rule sensitivity and
family-stratified outcomes under a frozen local system. The two scopes differ;
the current evidence does not justify claiming that Quanto omits every
structural question or that this project is the first ceiling/context study.
...
**What they did NOT do**: The micro-benchmark is a *measurement tool*, not an *optimization method* or a structural theory. Its six circuits and this project's 96,205 repeated rows use different statistical units, so an orders-of-magnitude scale comparison is invalid. The relevant distinction is research question and coverage, not raw row count.
...
**What they did NOT do**: Quasar's optimality is *step-limited* — optimal within the k-step closure of its rewrite set — not a structural statement about what fraction of a circuit is removable by a given rewrite class. The current project has not established that Quasar lacks an equivalent representation diagnostic without a feature-level full-paper audit. The remaining comparison is narrower: whether a frozen flat-listing/window/rule decomposition and sealed family-level prediction add explanatory evidence beyond Quasar's graph/sequence e-graphs.

### 10.9a Q-PreSyn: Learning Representation-Changing Edits Before Synthesis

**Citation**: Bosco, Cincio, Serra & Cerezo. *Quantum Circuit Pre-Synthesis: Learning Local Edits to Reduce T-count*, arXiv:2601.19738, 2026 [90].

**What they did**: Q-PreSyn learns sequences of equivalence-preserving local
edits that change the circuit representation presented to downstream
Clifford+T synthesis. Its premise directly states that composing local
synthesis methods can be suboptimal and that performance strongly depends on
the input representation.

**What they achieved**: The arXiv record reports reductions of up to 20% in
T-count on circuits up to 25 qubits without adding approximation error before
synthesis. These figures are reported-paper evidence, not results reproduced
by this repository.

**What they did NOT establish for this project**: Q-PreSyn targets downstream
T-count and a learned pre-synthesis policy, whereas this project freezes a
flat listing, local rule predicate, family-stratified mechanism, and sealed
out-of-family diagnostic. That difference may support a narrower contribution,
but Q-PreSyn rules out claiming novelty for representation dependence itself.
...
| Q-PreSyn | [90] | 2026 | $-$ | Partial (representation-conditioned synthesis) | $-$ | Partial (equivalence-preserving edits) | $-$ |
| SSR | [45] | 2026 | $-$ | $-$ | $-$ | Partial (SAT depth-optimality, subcircuit level) | $-$ |
| **This work (current evidence)** | — | 2026 | Partial (specified listing/rules only) | Sealed held-out test passed under its fixed label/feature contract | Partial (cluster-aware analyses being finalized) | Scoped Thm 7/9 only; no general ceiling theorem | 520-input four-tool contract complete; strong external methods pending |

**Key observations from Table 5**:

1. A broad **structural ceiling** novelty claim is not defensible: Quasar gives
   step-limited optimality and cut-and-meld gives a formal local-optimality
   criterion.  The unresolved question is whether the project's narrower,
   listing/model-conditional empirical diagnostic adds a useful explanatory
   or predictive layer.

2. Context dependence and representation choice are explicit in several prior
   systems (including Quarl, PCOAST, AlphaTensor-Quantum, Quasar, and
   Q-PreSyn).  The
   project's family-stratified estimates may be useful, but uniqueness remains
   unproved; the sealed eight-family held-out test supports transfer under its
   fixed contract but does not establish uniqueness.

3. **Systematic benchmark suites** at scale are rare. This project has 96,205 active-canonical repeated observations across 35 datasets, but statistical power depends on independent circuit instances and pairing, not row count; no two-orders priority inference follows.

4. **Formal theory** includes transformation verification (Quartz/Quanto/VOQC), tensor structure (AlphaTensor-Quantum), step-limited equality-saturation optimality (Quasar), and cut-and-meld local optimality (Arora et al. 2025). This project has not yet proved a general optimization-ceiling or average-case-complexity theorem.

5. The 520-input custom/Qiskit/Cirq/t|ket> rerun now satisfies the shared-input
   and exact-equivalence contract. It remains an empirical fixed-version
   benchmark; stronger external optimizers are still required for a meaningful
   boundary comparison.

### 10.12 Summary of Positioning

The eleven works surveyed above represent significant advances in *how* to optimize quantum circuits — through larger search spaces (Quartz, Quanto, Quasar), learned policies (Qarl, ZX+RL, Q-PreSyn), domain-specific RL (AlphaTensor-Quantum), relaxed matching (RPO), verified rewrites (VOQC), post-mapping depth recovery (SSR), and better tooling (Micro-Benchmark). The repository has not established that none addresses failure mechanisms; its remaining question is specifically whether the frozen flat-listing/rule decomposition yields an independently useful family-level diagnostic.

The defensible positioning is a **narrow boundary diagnostic** for a specified
flat listing and local rule set. A sealed eight-family held-out test now supports
out-of-family prediction under the frozen label/feature contract. Validation
against stronger optimizers remains incomplete, so an a-priori
resource-allocation claim is premature.
...
1. **Local optimization** has substantial classical and quantum theory; the
   unresolved niche here is quantitative boundary diagnosis for one explicit
   flat representation/rule system.
2. Standard **Non-Identity/Non-Equivalence Check** has QMA-complete promise
   formulations, but CODP/peephole-optimization hardness is open here.
...
4. The observed random-circuit zero under LBL is a representation/predicate
   effect; it is not evidence of Haar incompressibility.
5. Clifford semantic equivalence is efficient, but general optimal Clifford
   synthesis does not follow. v10 E23 validates zero Greedy reduction only for
   the corrected restricted AG-stage generator.
6. Recent frameworks already study local optimality, representation, search
   limits, and context in several forms. The remaining candidate novelty is the
   project's paired flat-listing sensitivity plus sealed predictive audit under
   a fully frozen empirical contract.

**Current evidence contributes** to the gap between theory and compiler
performance by:
- Quantifying a **representation- and rule-conditional empirical boundary** for Greedy/local rewriting
- Quantifying **family-dependent Phase-2 effects** and validating two scoped existence constructions
- Providing a large repeated-measures corpus (96,205 active-canonical rows, 35 datasets, experiments E1–E30), without a cross-study size-priority claim
- Establishing a **data integrity protocol** for reproducible quantum optimization research
- Providing a paired, exact-verified multi-compiler comparison under one fixed contract
- Maintaining a claim-status framework that explicitly separates proved, restricted, refuted, withdrawn, and empirical statements
