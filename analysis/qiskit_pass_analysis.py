...
DEPRECATION NOTICE (wave-4 code hygiene)
----------------------------------------
The canonical source of Figures 15-17 is now ``analysis/generate_figures.py``
(single entry point regenerating all 17 manuscript figures). It reuses this
module's data functions (load_data, compute_*, save_summary_csv) but applies
the unified wave-1 style: bootstrap 95% CI error bars, the Okabe-Ito
colorblind-safe palette, and perceptually uniform colormaps. The standalone
plotting functions below (generate_fig15_waterfall / generate_fig16_heatmap /
generate_fig17_interaction) are DEPRECATED: they still run for backward
compatibility but emit a DeprecationWarning, use the legacy SEM error bars and
legacy palette, and must not be used to produce manuscript figures. The data
functions are NOT deprecated and remain the shared computation layer.
"""
...
# ===================================================================
#
# DEPRECATED (wave-4): the three generate_fig1x_* functions below are the
# legacy standalone plotting path. The canonical figure source is
# analysis/generate_figures.py, which reproduces Figures 15-17 with the
# unified wave-1 style (bootstrap 95% CI instead of SEM, Okabe-Ito palette,
# perceptually uniform colormaps). These functions are kept callable for
# backward compatibility only and emit a DeprecationWarning when used.

_DEPRECATION_MSG = (
    "{name}() is deprecated: Figures 15-17 are canonically generated by "
    "analysis/generate_figures.py (unified style, bootstrap 95% CI). "
    "This legacy function uses SEM error bars and the old palette and must "
    "not be used for manuscript figures."
)


def _warn_deprecated_figure(name: str) -> None:
    """Emit a DeprecationWarning for a legacy standalone figure function."""
    warnings.warn(
...
def generate_fig15_waterfall(pass_df: pd.DataFrame,
    ...
    .. deprecated:: wave-4
        Canonical source is analysis/generate_figures.py (bootstrap 95% CI
        error bars, Okabe-Ito palette). This legacy version uses SEM bars.
    ...
    """
    _warn_deprecated_figure("generate_fig15_waterfall")
...
def generate_fig16_heatmap(pass_df: pd.DataFrame) -> None:
    ...
    .. deprecated:: wave-4
        Canonical source is analysis/generate_figures.py.

    Rows = circuit families, columns = individual passes.
    Cell value = mean gate reduction (%).
    """
    _warn_deprecated_figure("generate_fig16_heatmap")
...
def generate_fig17_interaction(pass_df: pd.DataFrame) -> None:
    ...
    """
    _warn_deprecated_figure("generate_fig17_interaction")
...
def main() -> None:
    ...
    # Generate figures
    # NOTE (wave-4): these calls use the DEPRECATED legacy plotting path
    # (SEM bars, legacy palette). For manuscript figures run
    # analysis/generate_figures.py instead, which is the canonical source.
    print("\n" + "=" * 60)
